...
fn bench_size_parsing(c: &mut Criterion) {
    ...
    c.bench_function("size_parsing_percentage", |b| {
        b.iter(|| {
            rt.block_on(async {
                let result = plugin
                    .parse_size(black_box("75% 60%"), black_box(&monitor), None)
                    .await
                    .unwrap();
                black_box(result);
            });
    ...
    c.bench_function("size_parsing_pixels", |b| {
        b.iter(|| {
            rt.block_on(async {
                let result = plugin
                    .parse_size(black_box("800px 600px"), black_box(&monitor), None)
                    .await
                    .unwrap();
                black_box(result);
            });
...
fn bench_dimension_parsing(c: &mut Criterion) {
    ...
    c.bench_function("dimension_parsing_percentage", |b| {
        b.iter(|| {
            let result = plugin
                .parse_dimension(black_box("75%"), black_box(1920))
                .unwrap();
            black_box(result);
        });
    });
    ...
    c.bench_function("dimension_parsing_pixels", |b| {
        b.iter(|| {
            let result = plugin
                .parse_dimension(black_box("800px"), black_box(1920))
                .unwrap();
            black_box(result);
        });
    });
    ...
    c.bench_function("dimension_parsing_raw", |b| {
        b.iter(|| {
            let result = plugin
                .parse_dimension(black_box("800"), black_box(1920))
                .unwrap();
            black_box(result);
        });
    });
        ...
        b.iter(|| {
            rt.block_on(async {
                ...
                let tasks = (0..10).map(|i| {
                    ...
                    async move {
                        plugin
                            .parse_size(&format!("{}% {}%", 50 + i, 40 + i), monitor, None)
                            .await
                            .unwrap()
                    }
                });
