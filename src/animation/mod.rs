...
pub use timeline::{Timeline, TimelineBuilder, AnimationDirection, Keyframe};
pub use properties::{PropertyValue, Color, Transform, AnimationProperty};
...
impl AnimationEngine {
    ...
    pub async fn start_animation(
        ...
        info!("🎬 Starting animation '{}' with type '{}'", id, config.animation_type);
        ...
        match config.animation_type.as_str() {
            "fromTop" => {
                ...
                if let Some(y) = targets.get("y") {
                    targets.insert("y".to_string(), PropertyValue::Pixels(
                        y.as_pixels() - offset_pixels as i32
                    ));
                }
            },
            "fromBottom" => {
            ...
            "fromLeft" => {
                ...
                if let Some(x) = targets.get("x") {
                    targets.insert("x".to_string(), PropertyValue::Pixels(
                        x.as_pixels() - offset_pixels as i32
                    ));
                }
            },
            "fromRight" => {
            ...
            "fade" => {
                targets.insert("opacity".to_string(), PropertyValue::Float(config.opacity_from));
            },
            "scale" => {
    ...
    async fn run_animation_loop(&mut self, animation_id: String) -> Result<()> {
        ...
        loop {
            ...
            let should_continue = {
                ...
                Self::interpolate_properties_for_animation(&mut self.active_animations, &animation_id, eased_progress)?;
    ...
    pub fn pause_animation(&mut self, animation_id: &str, paused: bool) -> Result<()> {
        if let Some(animation) = self.active_animations.get_mut(animation_id) {
            ...
            let action = if paused { "⏸️  Paused" } else { "▶️  Resumed" };
    ...
    pub fn get_current_properties(&self, animation_id: &str) -> Option<&HashMap<String, PropertyValue>> {
        self.active_animations.get(animation_id).map(|anim| &anim.properties)
    }
    
    /// Get performance statistics
    pub fn get_performance_stats(&self) -> PerformanceStats {
        let avg_frame_time = if !self.performance_monitor.frame_times.is_empty() {
            self.performance_monitor.frame_times.iter().sum::<Duration>() / self.performance_monitor.frame_times.len() as u32
        } else {
...
fn default_easing() -> String { "easeInOut".to_string() }
fn default_offset() -> String { "100%".to_string() }
fn default_scale() -> f32 { 0.0 }
fn default_true() -> bool { true }
fn default_spring_stiffness() -> f32 { 300.0 }
fn default_spring_damping() -> f32 { 30.0 }
