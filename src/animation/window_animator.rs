...
use super::{AnimationConfig, AnimationEngine, properties::PropertyValue};
...
impl WindowAnimator {
    ...
    pub async fn show_window(
        ...
        info!("🎬 Starting show animation for window {} with type '{}'", 
              window_address, config.animation_type);
        
        // Calculate starting position based on animation type
        let start_position = self.calculate_start_position(
            target_position,
            target_size,
            &config
        ).await?;
        ...
        self.set_window_properties(window_address, start_position, target_size, 0.0).await?;
        ...
        self.active_window_animations.insert(window_address.to_string(), state);
        ...
        if config.animation_type.contains("fade") {
            initial_properties.insert("opacity".to_string(), PropertyValue::Float(config.opacity_from));
        }
        ...
        engine.start_animation(animation_id.clone(), config, initial_properties).await?;
        
        // Start window update loop
        self.start_window_animation_loop(window_address.to_string(), animation_id).await?;
    ...
    pub async fn hide_window(
        ...
        info!("🎬 Starting hide animation for window {} with type '{}'", 
              window_address, config.animation_type);
        
        // Calculate ending position based on animation type
        let end_position = self.calculate_end_position(
            current_position,
            current_size,
            &config
        ).await?;
    ...
    async fn calculate_start_position(
        ...
        match config.animation_type.as_str() {
            ...
            "fromTopLeft" => Ok((-target_size.0 - offset_pixels, -target_size.1 - offset_pixels)),
            "fromTopRight" => Ok((screen_size.0 + offset_pixels, -target_size.1 - offset_pixels)),
            "fromBottomLeft" => Ok((-target_size.0 - offset_pixels, screen_size.1 + offset_pixels)),
    ...
    async fn start_window_animation_loop(&self, window_address: String, animation_id: String) -> Result<()> {
    ...
    async fn apply_properties_to_window(
        ...
        // Extract position
        let x = properties.get("x")
            .map(|p| p.as_pixels())
            .unwrap_or(0);
        let y = properties.get("y")
            .map(|p| p.as_pixels())
            .unwrap_or(0);
        ...
        let width = properties.get("width")
            .map(|p| p.as_pixels())
            .unwrap_or(800);
        let height = properties.get("height")
            .map(|p| p.as_pixels())
            .unwrap_or(600);
    ...
    async fn set_window_properties(
        ...
        client.move_window(window_address, position.0, position.1).await?;
