...
impl Timeline {
    ...
    pub fn remove_keyframe_at(&mut self, time: f32) {
        self.keyframes
            .retain(|k| (k.time - time).abs() > f32::EPSILON);
    }
    ...
    pub fn slide_timeline(duration: Duration, from_pos: f32, to_pos: f32) -> Self {
        let keyframes = vec![
            ...
            Keyframe {
                time: 0.2,
                value: from_pos + (to_pos - from_pos) * 0.1,
                easing: Some("ease-in".to_string()),
            },
            Keyframe {
                time: 0.8,
                value: from_pos + (to_pos - from_pos) * 0.9,
                easing: Some("ease-out".to_string()),
            },
            Keyframe {
                time: 1.0,
...
impl TimelineBuilder {
    ...
    pub fn keyframe(mut self, time: f32, value: f32, easing: Option<&str>) -> Self {
        self.timeline
            .add_keyframe(time, value, easing.map(|s| s.to_string()));
        self
    }
