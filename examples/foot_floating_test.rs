use rustrland::animation::{window_animator::WindowAnimator, AnimationConfig};
...
async fn demo_manual_directional() -> anyhow::Result<()> {
    ...
    animator
        .spawn_window_offscreen("foot", 560, -600, 800, 600)
        .await?;
    ...
    if let Some(window) = foot_window {
        ...
        println!(
            "📍 Current position: ({}, {}) (should be off-screen)",
        ...
        println!(
            "   Animating from Y={} to Y={} (should appear from top of screen)",
            start_y, target_y
        );
        ...
        for frame in 0..total_frames {
            ...
            if frame % 8 == 0 {
                // Print every 8th frame (~every 130ms)
                println!(
                    "Frame {:2}: progress={:.2}, eased={:.3}, Y={}px (overshoot!)",
...
async fn demo_animator_directional() -> anyhow::Result<()> {
    ...
    println!(
        "Config > animation: {} - duration: {} - easing: {}",
    ...
    let windows = animator
        .show_window("foot", (880, 420), (800, 600), config)
        .await?;
