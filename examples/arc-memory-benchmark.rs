...
fn benchmark_memory_usage() {
    ...
    println!("  Arc size reduction: {:.1}x smaller", traditional_size as f64 / arc_size as f64);
    ...
    for i in 0..5 {
        ...
        println!("  Traditional plugin {}: {} bytes (+ config data)", i + 1, mem::size_of_val(&traditional_plugins[i]));
    }
    ...
    for i in 0..5 {
        ...
        println!("  Arc plugin {}: {} bytes (shares config data)", i + 1, mem::size_of_val(&arc_plugins[i]));
    }
    ...
    println!("  Memory reduction: {:.1}% ({:.1}x smaller)", 
...
fn benchmark_performance() {
    ...
    println!("\n📊 Memory Growth Analysis:");
    println!("  Traditional plugin cache size: {} items × {} bytes = {} KB",
    ...
    println!("  Arc plugin cache size: {} items × {} bytes = {} bytes",
    ...
    println!("  Strong references to config: {}", Arc::strong_count(&arc_plugin.config));
    println!("  Weak references to config: {}", Arc::weak_count(&arc_plugin.config));
}
...
fn demonstrate_sharing_benefits() {
    ...
    println!("  Total config memory: {} bytes (would be {} bytes with cloning)",
...
fn real_world_simulation() {
    ...
    for plugin_name in &["scratchpads", "workspaces", "expose", "magnify"] {
        ...
        println!("  {} plugin: {} bytes", plugin_name, mem::size_of_val(&plugin_config));
    }
