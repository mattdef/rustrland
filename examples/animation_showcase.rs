...
async fn demo_basic_directional(engine: &mut AnimationEngine, client: &HyprlandClient, animator: &mut WindowAnimator) -> anyhow::Result<()> {
    ...
    animator.spawn_window_offscreen("foot", 200, -600, 800, 600).await?;
    ...
    if let Some(window) = foot_window {
        println!("   ✅ Found foot window at ({}, {})", window.at.0, window.at.1);
        ...
        println!("   🎬 Animating fromTop: {} → {} with ease-out-cubic", start_y, target_y);
        ...
        for frame in 0..total_frames {
            ...
            if frame % 6 == 0 {
                println!("   Frame {}: Y = {}px (progress: {:.2})", frame, current_y, progress);
            }
...
async fn demo_spring_physics(engine: &mut AnimationEngine, client: &HyprlandClient, animator: &mut WindowAnimator) -> anyhow::Result<()> {
    ...
    if let Some(window) = foot_window {
        ...
        println!("   🔬 Spring physics animation: {} → {} with bounce", start_y, target_y);
        ...
        for frame in 0..total_frames {
            ...
            if frame % 12 == 0 {
                println!("   Frame {}: Y = {}px (spring physics: {:.3})", frame, current_y, eased_progress);
            }
...
async fn demo_multi_property(engine: &mut AnimationEngine, client: &HyprlandClient, animator: &mut WindowAnimator) -> anyhow::Result<()> {
    ...
    animator.spawn_window_offscreen("thunar", 200, -600, 800, 600).await?;
    ...
    if let Some(window) = thunar_window {
        println!("   ✅ Found thunar window at ({}, {})", window.at.0, window.at.1);
        let address = window.address.to_string();
        ...
        println!("   🎯 Multi-property animation: position ({},{}) → ({},{}) with ease-out-back",
        ...
        for frame in 0..total_frames {
            ...
            if frame % 6 == 0 {
                let overshoot_info = if eased_progress > 1.0 { " ← OVERSHOOT!" } else { "" };
                println!("   Frame {}: ({}, {}) eased={:.3}{}",
...
async fn demo_custom_easing(engine: &mut AnimationEngine, client: &HyprlandClient, animator: &mut WindowAnimator) -> anyhow::Result<()> {
    ...
    if let Some(window) = foot_window {
        ...
        println!("   🎨 Demonstrating ease-out-bounce: {} → {}", start_y, target_y);
        ...
        for frame in 0..total_frames {
            ...
            client.move_window_pixel(&address, 300, current_y).await.ok();
            
            if frame % 8 == 0 {
                println!("   Frame {}: Y={}px bounce={:.3}", frame, current_y, eased_progress);
            }
...
async fn demo_animation_sequence(engine: &mut AnimationEngine, client: &HyprlandClient, animator: &mut WindowAnimator) -> anyhow::Result<()> {
    ...
    if let Some(window) = foot_window {
        ...
        for frame in 0..30 { // 500ms
            let progress = frame as f32 / 29.0;
            let eased = easing2.apply(progress);
            let current_x = start_x + (distance2 as f32 * eased) as i32;
            client.move_window_pixel(&address, current_x, target_y1).await.ok();
            
            if frame % 8 == 0 {
                println!("   Phase 2 Frame {}: X={}px (bounce={:.3})", frame, current_x, eased);
            }
            sleep(Duration::from_millis(16)).await;
        }
        ...
        for frame in 0..18 { // 300ms
            ...
            client.move_window_pixel(&address, target_x2, current_y).await.ok();
...
async fn demo_performance_monitoring(engine: &mut AnimationEngine, client: &HyprlandClient, animator: &mut WindowAnimator) -> anyhow::Result<()> {
    ...
    animator.spawn_window_offscreen("foot", 100, -600, 800, 600).await?;
    animator.spawn_window_offscreen("foot", 200, -600, 800, 600).await?;
    animator.spawn_window_offscreen("foot", 300, -600, 800, 600).await?;
    ...
    let foot_windows: Vec<_> = windows.iter()
        .filter(|w| w.class.to_lowercase().contains("foot"))
        .cloned()
        .collect();
    
    if foot_windows.len() >= 3 {
        println!("   ✅ Found {} foot windows for performance test", foot_windows.len());
        ...
        for frame in 0..total_frames {
            ...
            if let Some(window1) = foot_windows.get(0) {
                ...
                client.move_window_pixel(&window1.address.to_string(), 200, current_y).await.ok();
            }
            ...
            if let Some(window2) = foot_windows.get(1) {
                ...
                let start_y = -400;
                let current_y = start_y + ((target_y - start_y) as f32 * eased) as i32;
                client.move_window_pixel(&window2.address.to_string(), 400, current_y).await.ok();
            }
            ...
            if let Some(window3) = foot_windows.get(2) {
                ...
                let start_y = -400;
                let current_y = start_y + ((target_y - start_y) as f32 * eased) as i32;
                client.move_window_pixel(&window3.address.to_string(), 600, current_y).await.ok();
            }
            ...
            if frame % 10 == 0 {
                ...
                println!("   Frame {}: {:.1}ms ({:.0} FPS) - {} windows animated", 
                        frame, frame_time.as_millis(), current_fps, foot_windows.len());
            }
        ...
        println!("      • Total Frames Rendered: {}", total_frames * foot_windows.len());
        ...
        for window in &foot_windows {
            animator.close_window(&format!("{}", window.address)).await?;
        }
        
        println!("   🧹 Cleaned up {} performance test windows", foot_windows.len());
        println!("   ✅ Performance test completed - system handled {} simultaneous animations!", foot_windows.len());
    } else {
